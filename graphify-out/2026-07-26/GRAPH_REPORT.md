# Graph Report - release-deflock-field-dossier-operator  (2026-07-26)

## Corpus Check
- 286 files · ~260,728 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3360 nodes · 5877 edges · 214 communities (193 shown, 21 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 159 edges (avg confidence: 0.64)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9ba2175f`
...
- wardriver-release-current-api.test.mjs
...
1. `$()` - 32 edges
2. `_run_tick_locked()` - 31 edges
3. `fresh_snapshot()` - 30 edges
4. `AutonomousPaperEngineTests` - 26 edges
5. `MemoryObservationStore` - 24 edges
6. `buildDomainPayload()` - 23 edges
7. `invalid()` - 23 edges
8. `PostgresObservationStore` - 22 edges
9. `validatePaperState()` - 22 edges
10. `renderArHud()` - 21 edges

## Surprising Connections (you probably didn't know these)
- `loadTzeentchMarketFeed()` --indirect_call--> `headers()`  [INFERRED]
  app/operator/tzeentch.mjs → vm/cybermap-api/test/morning-brief.test.mjs
...
- `renderCarousel()` --indirect_call--> `unavailable()`  [INFERRED]
  app/operator/morning-brief.mjs → api/cybermap-global-viewport/index.js
...
Cohesion: 0.10
Nodes (64): append_jsonl(), canonical_json_hash(), clean_string(), collect_source_snapshot(), configured_paper_sync_url(), cron_packet(), default_ledger(), fragment_body() (+56 more)
...
Cohesion: 0.06
Nodes (33): closeBoundary(), DEFLOCK_H3_RESOLUTIONS, materializeDeflockReports(), normalizePoint(), validTimestamp(), main(), runDeflockSourceProcess(), boundedCount() (+25 more)

### Community 9 - "wigle.mjs"
Cohesion: 0.10
Nodes (47): applyWigleDataset(), metersPerPixel(), annotateCurrentRecord(), bearingBetween(), buildArCandidateBoxes(), buildCurrentWigleState(), buildWigleMapState(), categorizeSignal() (+39 more)
...
Cohesion: 0.09
Nodes (25): aggregateCaveats(), aggregateFreshness(), aggregateLayers(), aggregateSourceClasses(), boundedGlobalLimit(), finiteInteger(), finiteOrNull(), GLOBAL_SOURCE_CLASSES (+17 more)

### Community 17 - "main.js"
Cohesion: 0.11
Nodes (38): activateTab(), activateTabByIndex(), bindLoginFlow(), bindMorningBriefReturn(), bindOperatorDownloads(), bindTabSystem(), buildOperatorHeaders(), clearOperatorSession() (+30 more)
...
Cohesion: 0.08
Nodes (17): pool, port, server, store, createCybermapApiServer(), request, GlobalViewportStore, VALID_REQUEST (+9 more)

### Community 29 - "server.mjs"
Cohesion: 0.08
Nodes (29): canonicalJson(), clampFiniteNumber(), GLOBAL_VIEWPORT_LAYER_IDS, handleCybermapViewport(), PAPER_ACTION_KEYS, PAPER_ARCHIVE_KEYS, PAPER_BOOK_COST_KEYS, PAPER_BOOK_IDS (+21 more)
...
Cohesion: 0.12
Nodes (12): forbidden(), hashToken(), tokenDigestMatches(), ContractError, NOW, validBatch(), validObservation(), createStore() (+4 more)

### Community 37 - "operator-auth.js"
Cohesion: 0.14
Nodes (21): { requireOperatorToken }, base64UrlDecode(), base64UrlEncode(), buildClearOperatorSessionCookie(), createOperatorToken(), crypto, extractCookieValue(), extractOperatorTokens() (+13 more)
...
}, buildRunId(), commit(), createReleaseStore(), isoTimestamp(), notes(), parseConnectionString(), positiveInt() (+12 more)

### Community 43 - "refreshLiveWigleFeed"
Cohesion: 0.14
Nodes (24): activateGodeyeMode(), bindDeflockGlobalControls(), bindGodeyeModeControls(), bindWigleControls(), buildDeflockGlobalRequest(), buildWigleRequestPayload(), getCurrentPosition(), handleGeoError() (+16 more)
...
Cohesion: 0.12
Nodes (10): Approved scope, Provider review, Server-side configuration, TST-009 controlled validation, USGS Earthquake Hazards Terms Review and Controlled Enablement Receipt, Former topology, Historical VM echo wiring (retired), Apply locally (+2 more)
...
Cohesion: 0.22
Nodes (18): buildEchoPayload(), createRequestHandler(), handleGlobalViewport(), handleMorningBriefRequest(), handleMorningBriefWrite(), handleObservationBatch(), handlePaperStateRead(), handlePaperStateWrite() (+10 more)
...
Cohesion: 0.51
Nodes (12): collect_market_snapshot(), fetch_json(), held_prediction_market_ids(), iso_z(), normalize_iso(), parse_cboe_quote(), parse_coingecko_markets(), parse_json_list() (+4 more)
...
Cohesion: 0.15
Nodes (8): appRoot, brief, execFileAsync, fixturePng, MIME_TYPES, nacreCss, operatorShell, repoRoot
...
Cohesion: 0.18
Nodes (9): ingestHeaders(), canonicalPaperState(), legacyPaperStateV2(), PAPER_BOOK_IDS, PAPER_LINES, PAPER_NOW_MS, PAPER_STRATEGIES, paperProfile() (+1 more)
...
### Community 97 - "Data Model and Contracts: Godeye Global Map"
Cohesion: 0.18
...
Cohesion: 0.27
Nodes (5): hashCanonicalJson(), hashPersistedObservation(), insertObservations(), normalizeDatabaseError(), touchCredential()
...
### Community 119 - "clamp"
Cohesion: 0.46
Nodes (6): getArVideoScale(), buildTileGrid(), clamp(), latLonToTileXY(), tileUrl(), wrapTileIndex()
...
### Community 138 - "Blue Swallow Society on Azure Static Web Apps + Cybermap Backend"
Cohesion: 0.29
...
### Community 183 - "Reconciled Architecture"
Cohesion: 0.50
...
### Community 185 - "wardriver-release-current-api.test.mjs"
Cohesion: 0.50
Nodes (3): handler, release, require
...
## Knowledge Gaps
- **1272 isolated node(s):** `check-prerequisites.sh script`, `common.sh script`, `create-new-feature.sh script`, `setup-plan.sh script`, `setup-tasks.sh script` (+1267 more)
...
- **Why does `renderCarousel()` connect `morning-brief.mjs` to `index.js`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `unavailable()` connect `index.js` to `morning-brief.mjs`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `renderRecentQueries()` connect `tzeentch.mjs` to `morning-brief.mjs`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Are the 9 inferred relationships involving `_run_tick_locked()` (e.g. with `RuntimeError` and `held_prediction_market_ids()`) actually correct?**
