...
test('VM Cybermap installer applies checked-in SQL migrations with psql', () => {
  ...
  assert.match(installCybermapApi, /printf 'DATABASE_URL=postgresql:\/\/__POSTGRES_ADMINISTRATOR_LOGIN__:%s@__POSTGRES_SERVER_FQDN__:5432\/__POSTGRES_DATABASE_NAME__\?sslmode=verify-full\\n' "\$POSTGRES_PASSWORD_URLENCODED"/);
  assert.doesNotMatch(installCybermapApi, /DATABASE_URL=.*\*\*\*/);
