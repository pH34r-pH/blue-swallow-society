...
test('operator entrypoint requires an existing passcode-issued session before showing the console', () => {
  ...
  assert.ok(operatorLoaderJs.includes("'styles.css'"));
  assert.ok(operatorLoaderJs.includes("'theme.css'"));
  assert.ok(operatorLoaderJs.includes('preparePrivateAssets(session)'));
  assert.ok(operatorLoaderJs.includes('fetchPrivateAsset(assetName, session)'));
  assert.ok(operatorLoaderJs.includes('headers: operatorHeaders(session)'));
  assert.match(operatorLoaderJs, /URL\.createObjectURL\(new Blob\(\[source\], \{ type \}\)\)/, 'private modules must remain in transient memory');
  assert.match(operatorLoaderJs, /URL\.revokeObjectURL\(url\)/, 'private module URLs must be revoked on auth loss or page exit');
  assert.match(operatorLoaderJs, /await Promise\.all\(\[\s*response\.text\(\),\s*preparePrivateAssets\(session\),\s*\]\)/, 'the private shell must wait for authenticated asset preparation before rendering');
  assert.ok(operatorLoaderJs.includes("import(assetUrls['main.js'])"));
