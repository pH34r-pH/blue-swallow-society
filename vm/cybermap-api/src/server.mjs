  ...
  return async function requestHandler(request, response) {
    try {
      ...
      if (request.method === 'POST' && url.pathname === OPERATOR_SIGNALS_PATH) {
        ...
        const snapshot = await handleOperatorSignalSnapshotPost(request, { store, now });
      ...
      if (request.method === 'POST' && url.pathname === VIEWPORT_PATH) {
        ...
        const viewport = await handleCybermapViewportPost(request, { store, now });
...
async function handleOperatorSignalSnapshotPost(request, { store, now }) {
  ...
  return readOperatorSignalSnapshotFromBody(payload, { store, now });
}

async function handleCybermapViewportPost(request, { store, now }) {
  const payload = await readJsonRequestBody(request);
  return readViewportFromBody(payload, { store, now });
}
