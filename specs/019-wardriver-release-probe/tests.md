...
| TST-019-02 | Node unit | FR-004–006, NFR-001/003 | Invoke the authorized handler with a validated bss.26-shaped release-store double and project a legacy manifest that omits acceptance mode. | Exact private/no-store allowed-key projection, including blob name and accepted mode; legacy projection exposes `acceptanceMode: null`; no SAS/download/session fields. | `tests/wardriver-release-probe-api.test.mjs` |
