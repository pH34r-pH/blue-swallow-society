...
- [ ] T005 Create policy fixtures in `tests/fixtures/godeye-policy-data.mjs` for permitted Cybermap observations, six independently missing operation/context-aware conflict gates, healthy/degraded/offline/unknown sources, trusted-clock delay boundaries, server coarsening, strategic objects, clusters, low-precision events, retractions, Murmurs signals, and prohibited active-force records.
- [ ] T006 [P] [US2] Create failing URL/default/history tests in `tests/godeye-view-state.test.mjs` covering canonical deep links without disabled layers, invalid-view fallback, `cybermap` 0-20/`conflicts` 0-10 zoom clamps, cross-view layer rejection, Back/Forward restoration, and forbidden serialization of coordinates/credentials/RF ids.
- [ ] T007 [P] [US3] Create failing source-gate tests in `tests/godeye-source-gates.test.mjs` proving all six gates are mandatory per operation/context, every failure reason is returned, caller time cannot satisfy delay, degraded health permits cached display but not fetch, unknown fails closed, P0 `enabled` remains false even when descriptive metadata is eligible, the registry exposes no conflict adapter callable/endpoint/override, and no conflict fetch occurs.
- [ ] T008 [P] [US4] Create failing zoom/safety tests in `tests/godeye-map-policy.test.mjs` covering zoom 0-4/5-7/8-10, >10 normalization, monotonic effective coarsening, Murmurs non-assertion, and active-force exclusion at every zoom.
- [ ] T009 [P] [US1] Modify `tests/security-review.test.mjs` with failing assertions for no browser camera/RF/loopback Godeye path, no runtime demo seed, no public-root/operator leak, no browser mutation/analytics/telemetry request, `lat`/`lon` excluded from all URL state, no direct third-party tile/glyph/sprite host, and no third-party script/style CSP origin.
...
- [ ] T016 Modify `tests/security-review.test.mjs` to verify the exact pin, license, self-hosted paths, same-origin-only browser map CSP, and absence of direct public tile hosts; run `npm run vendor:maplibre -- --check` and the focused security test. Record the approved same-origin/self-hosted basemap license, privacy, cache, abuse-control, and log-retention decision before rollout.
...
- [ ] T018 [US3] Create `app/operator/godeye-sources.mjs` with namespaced Cybermap/conflict registries, record/source-role classes, operation/context-aware all-six-gate evaluation, all-reason reporting, P0 hard-disabled runtime state, fetch/cache/display health decisions, and cross-view layer rejection; make T007 pass.
- [ ] T019 [US4] Add the strategic zoom/detail predicate to `app/operator/godeye-sources.mjs`, including conflict camera/detail max zoom 10, >10 normalization, separate epistemic/safety/zoom coarsening fields, monotonic effective display coarsening, Murmurs restrictions, and unconditional active-force exclusions; make T008 pass.
...
- [ ] T021 Create `app/operator/godeye-map.mjs` with one MapLibre instance, north-up defaults, the approved self-hosted/same-origin basemap style only, control installation, namespaced atomic source/layer reconciliation, resize, selection summary, and teardown. Fail unavailable when no approved basemap exists; keep business/source gate decisions outside MapLibre callbacks.
...
- [ ] T024 [P] [US1] Modify `tests/cybermap-viewport-api.test.mjs` and `vm/cybermap-api/test/http.test.mjs` to require POST-body-only coordinates on both hops, fixed backend endpoint, rejection of caller `now`/unknown fields, server-owned freshness time, token failure, 25-5000 m radius, 1-500 limit, no-store response, HTTPS backend, bounded local failures, effective rate/abuse control, and location-free log events.
- [ ] T025 [P] [US1] Modify `tests/security-review.test.mjs` to prove Godeye imports no camera/vision/RF scan/loopback capture API, emits no mutation/analytics/telemetry or direct third-party map request, and leaves the public root unchanged.
...
- [ ] T027 [US1] Modify `app/operator/main.js` to create the Godeye controller, initialize the shared map only inside the authenticated Godeye tab, request geolocation only after the existing explicit action, send exact coordinates only to the fixed same-origin read-query POST, use only the approved same-origin basemap path, and emit no browser mutation/analytics/telemetry request.
- [ ] T028 [US1] Modify `api/_private/operator/shell.html` to replace the manual tile/marker internals and operator-editable endpoint field with one map container plus Cybermap status/layer/inspector regions while preserving private-shell delivery.
...
- [ ] T031 [US1] Modify `api/cybermap-viewport/index.js` and `vm/cybermap-api/src/server.mjs` to use a fixed token-gated POST body on the private hop, reject unknown fields/caller `now`, use a server-owned/internally injected clock, disable the coordinate-bearing VM GET compatibility path before rollout, map upstream failures to bounded local classes, and add effective rate/abuse controls plus location-free event-class logging while preserving bounds, HTTPS, timeout, and no-store behavior.
...
- [ ] T039 [US2] Modify/verify `app/staticwebapp.config.json` keeps `/operator` deep links reachable, public navigation fallback unchanged, and browser map connections same-origin; remove direct public tile origins and add no permissive CSP allowance.
...
- [ ] T045 [US3] Modify `app/operator/styles.css` with separate Cybermap signal-strength and access-risk/source-class channels plus separate disabled/degraded/Mosaic/Murmurs semantics; conflict states must reuse neither Cybermap channel.
...
- [ ] T046 [US4] Modify `tests/godeye-map-policy.test.mjs` with table-driven coverage for every record class and zoom band through max 10, >10 normalization, a passing individual event, one failure for each operation-license/health/uncertainty/trusted-delay/coarsening/safety gate, and monotonic effective display coarsening.
...
- [ ] T055 [US5] Modify `app/operator/styles.css` with independent icon/shape/border/pattern/opacity channels and text labels for Cybermap signal strength, Cybermap access risk/source class, conflict type, verification, severity, precision, freshness, and Murmurs signals.
...
- [ ] T057 [P] Modify `tests/security-review.test.mjs` to verify no precise map/body data enters either network URL or logs, caller clocks/unknown fields are rejected, VM freshness time is server-owned, auth errors clear state, endpoint overrides are absent, browser map traffic is same-origin with no mutation/analytics/telemetry request, and conflict callables/endpoints cannot be introduced or enabled by URL/environment alone.
...
- [ ] T060 Verify `docs/godeye-web-architecture.md`, `specs/005-godeye-global-conflict-map/spec.md`, `plan.md`, and this task list still match source paths, auth, both API method/body rules, server-owned time, defaults, same-origin basemap, read-only behavior, no-adapter P0 state, and Constitution Check after implementation; run spec-compliance review before code-quality/security review, and block rollout unless inherited OAuth/OIDC plus credential-revocation/session-invalidation gates have an approved implemented resolution.
...
- [ ] T063 Run browser smoke plus authenticated DAST/manual abuse cases at 320/768/1024/1440 px for both views, keyboard-only navigation, reduced motion, auth/bounds/rate-limit failures, server-clock/private-hop URL/log/bounded-error privacy, same-origin-only basemap traffic, location denial, zero results, filter empty, stale source, partial failure, slow response after switch, Back/Forward, and logout/session expiry with rejection of the invalidated credential; include exact evidence in the review handoff.
...
- [ ] T066 Rerun T061-T064 plus `git diff --check`; expected: all pass and no runtime reference to the legacy renderer, coordinate-bearing private-hop GET, caller-controlled freshness clock, or direct/unbounded public tile service remains.
...
4. region/source-specific delay policy enforced with a trusted server clock;
5. server-side minimum coarsening radius/grid and zoom-generalization policy with epistemic uncertainty retained separately;
...
| FR-008-FR-012, FR-030, FR-035-FR-036 (field boundary, read-only browser, auth/public silence, no demo, same-origin basemap/private hop) | T009, T016, T024-T025, T057, T063 | T015-T016, T027-T031, T039, T060 |
...
| NFR-004, NFR-007-NFR-008 (sanitized errors/logs, API abuse controls, auth/session rollout gate) | T024-T025, T057, T063 | T031, T056, T060 |
...
| SC-008-SC-012 | T022-T025, T050-T051, T058, T061-T064 | T026-T031, T035-T055, T060, T065-T068 |
