...
3. Extend the Node request router with explicit mTLS-only routes: POST viewport/observations/feedback, plus header-validated GET catalog and artifact reads. Validate the trusted proxy marker before parsing a body; GET model reads permit only `channel=field` for catalog and use bounded compatibility headers. Keep the existing GET viewport and token paths unchanged.
