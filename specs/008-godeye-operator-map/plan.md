...
Create `api/_private/operator/assets/godeye-layers.mjs` with a frozen registry. The initial entries are:
...
Create `api/_private/operator/assets/godeye-map.mjs` as a lifecycle-controlled controller. It dynamically imports `/api/operator-assets/maplibre-gl.mjs` and loads CSS once, initializes MapLibre with a BSS-owned OSM raster style, adds MVT and GeoJSON sources, attaches operator headers only for same-origin BSS tile requests, and clears source data on stop. It has no remote fallback.

`api/_private/operator/assets/godeye-session-analysis.mjs` remains pure and derives no more than 24 timeline entries, source class counts, freshness, and empty/error states. The private main operator module supplies already-authorized data and clears it on logout/tab stop.
...
Vendor the pinned MapLibre ESM/CSS/license in `api/_private/operator/assets/`. Record version/source/tarball integrity in `api/_private/operator/assets/MAPLIBRE-VENDOR.md`. The operator-asset manifest serves only named assets after its short-lived private grant. Extend CSP only for self-hosted MapLibre worker creation and existing OSM raster network needs; no remote script/style permission is allowed.
...
app/staticwebapp.config.json               # routes + narrowed CSP changes

api/
├── cybermap-tiles/
│   ├── function.json
│   └── index.js
├── operator-assets/index.js               # private manifest delivery
└── _private/operator/
    ├── shell.html
    └── assets/
        ...
        ├── maplibre-gl.{mjs,css}
        ├── maplibre-gl-{shared,worker}.mjs
        └── maplibre-gl-LICENSE.txt / MAPLIBRE-VENDOR.md
