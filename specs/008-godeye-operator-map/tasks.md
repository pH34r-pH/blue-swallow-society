...
- [x] T006 [US1] [TST-001 GREEN] Implement frozen static layer policy and safe layer-only URL state in `app/operator/godeye-layers.mjs`.
- [x] T007 [US4] [TST-002 GREEN] Implement bounded no-persistence session reducer in `app/operator/godeye-session-analysis.mjs`.
...
- [x] T015 [US3] [TST-006/TST-007 GREEN] Vendor the pinned MapLibre JS/CSS/license/provenance under `app/operator/vendor/`; implement `app/operator/godeye-map.mjs` with same-origin tile auth, no remote fallback, safe MVT/GeoJSON sources, selected-cell callback, and clear lifecycle.
...
- [x] T017 [US3] [TST-006/TST-007 GREEN] Wire the controller and session reducer into `app/operator/main.js`; remove manual tile-grid/marker renderer imports and clear analysis/map data on Godeye stop/logout.
- [x] T018 [US3] [TST-006/TST-009 GREEN] Add responsive workbench styles in `app/operator/styles.css` and minimal MapLibre CSP allowances in `app/staticwebapp.config.json`.
