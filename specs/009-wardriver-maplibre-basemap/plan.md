...
2. Produce a MapLibre v8 style from `basemap/style.template.json`. The checked-in template has no remote tile origin and declares source `maxzoom: 12`; publication generates no tiles above zoom 12 so clients overscale the bounded static source rather than issuing unbounded high-zoom tile requests. Publication renders a generation-specific BSS static-website tile base from the website origin and one `wardriver-basemap` prefix into the style.
