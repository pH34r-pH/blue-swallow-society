...
| TST-017-04 | FR-001/003/004 | Postgres scripted store asserts scoped lock/query/insert SQL and a same-source key already held by another producer. | Device identity participates in every observation-identity operation; cross-producer reuse is accepted. |
| TST-017-05 | FR-002/006 | Root schema/installer contract inspects migration and registrations. | Forward-only scope-record creation without ledger updates; the migration rejects new batch-linked rows lacking producer scope; old uniqueness removal, new constraints, installer/readiness/docs registration. |
