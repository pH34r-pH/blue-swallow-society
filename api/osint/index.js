...
module.exports = async function (context, req) {
  const query = toCleanString(req?.query?.query ?? req?.body?.query ?? '').trim();
  const requestedMode = toCleanString(req?.query?.mode ?? req?.body?.mode ?? 'auto')
    .trim()
    .toLowerCase();
  const limit = clampNumber(parseInt(req?.query?.limit ?? req?.body?.limit ?? '5', 10), 1, 10);
...
async function probePublicUrl(targetUrl) {
...
function isUnsafeHostName(hostname) {
  const host = toCleanString(hostname).trim().toLowerCase();
  if (!host) return true;
...
function isPrivateIp(value) {
  if (net.isIP(value) === 4) {
    const [a, b] = value.split('.').map((segment) => parseInt(segment, 10));
    if (a === 10) return true;
...
async function fetchText(url, { timeout = DEFAULT_TIMEOUT_MS, headers = {}, method = 'GET' } = {}) {
  const response = await fetchWithTimeout(url, { timeout, headers, method });
...
async function fetchWithTimeout(url, { timeout, headers, method }) {
